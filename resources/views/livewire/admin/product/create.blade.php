<div class="space-y-6">
    ...
    <div>
        ...
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Add New Product</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Fill in the details to add a product to inventory
        </p>
    </div>
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <section class="bg-white dark:bg-gray-800 rounded-2xl shadow-card p-5 space-y-4">
            ...
            <div>
                ...
                <input id="prod-name" type="text" wire:model.blur="name" class="field @error('name') error @enderror"
                    placeholder="e.g. Samsung 55″ QLED TV" autocomplete="off">
                @error('name')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="prod-sku" type="text" wire:model.blur="sku"
                        class="field @error('sku') error @enderror" placeholder="e.g. TV-SAM-001">
                    @error('sku')
                        <p class="mt-1 text-xs text-red-500" role="alert">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <select id="prod-category" wire:model="category_id"
                        class="field @error('category_id') error @enderror">
        ...
        {{-- Stock card --}}
        <section class="bg-white dark:bg-gray-800 rounded-2xl shadow-card p-5 space-y-4">
            <h2 class="text-sm font-bold text-gray-900 dark:text-white">Stock</h2>

            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    <label for="prod-qty" class="block text-xs font-semibold text-gray-600 mb-1.5">
                        Quantity <span class="text-red-500" aria-hidden="true">*</span>
                    </label>
                    <input id="prod-qty" type="number" min="0" wire:model.blur="qty"
                        class="field @error('qty') error @enderror" placeholder="0">
                    @error('qty')
                        <p class="mt-1 text-xs text-red-500" role="alert">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input id="prod-reorder" type="number" min="0" wire:model.blur="reorder_level"
                        class="field @error('reorder_level') error @enderror" placeholder="1">
                    @error('reorder_level')
                        <p class="mt-1 text-xs text-red-500" role="alert">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <section class="bg-white dark:bg-gray-800 rounded-2xl shadow-card p-5 space-y-4">
            ...
            <label for="prod-image"
                class="flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-600 p-6 text-center cursor-pointer hover:border-brand-300 transition-colors">
                @if ($image)
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('products.index') }}" wire:navigate
                class="px-5 py-2.5 border border-gray-200 hover:bg-gray-50 text-gray-600 text-sm font-semibold rounded-xl transition-colors min-h-[44px] flex items-center">
                Cancel
            </a>
            <button type="submit" wire:loading.attr="disabled" wire:target="save"
                class="flex items-center justify-center gap-2 px-5 py-2.5 bg-brand-600 hover:bg-brand-700 disabled:opacity-60 text-white text-sm font-semibold rounded-xl transition-colors shadow-sm min-h-[44px]">
